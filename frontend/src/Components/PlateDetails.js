...
import {getPlateDetails} from '../Services/Actions/plateAction';
...
const PlateDetails = () => {
  ...
  useEffect(()=>{
    dispatch(getPlateDetails(_id))
  },[dispatch])

  const {plate, loading, error} = useSelector((state)=> state.platedetails)

  let product = plate.plate;
  console.log(product);
  ...
  <h2>Description</h2>
  {
    ...
    <div><span>Plate Type : </span><span>{product.plateType}</span> </div><hr/>
